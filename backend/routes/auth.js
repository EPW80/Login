...
const requestLogger = (req, res, next) => {
  ...
  if (req.body && Object.keys(req.body).length > 0) {
    ...
    if (logBody.signature) {
      logBody.signature =
        logBody.signature.substring(0, 10) +
        "..." +
        logBody.signature.substring(-10);
    }
  ...
  res.send = function (data) {
    ...
    try {
      ...
      if (logResponse.accessToken) {
        logResponse.accessToken =
          logResponse.accessToken.substring(0, 20) + "...";
      }
      if (logResponse.refreshToken) {
        logResponse.refreshToken =
          logResponse.refreshToken.substring(0, 20) + "...";
      }
...
const successLogger = (req, res, next) => {
  res.on("finish", () => {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      console.log(
        `✅ SUCCESS: ${req.method} ${req.path} - ${res.statusCode} (${req.requestId})`
      );
    } else if (res.statusCode >= 400) {
      console.log(
        `❌ ERROR: ${req.method} ${req.path} - ${res.statusCode} (${req.requestId})`
      );
    }
  });
  next();
